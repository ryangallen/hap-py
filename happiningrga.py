...
class BoardHome(BoardHandler):
	def get(self):
		happinings = db.GqlQuery("SELECT * FROM Happining ORDER BY created DESC LIMIT 10")
		self.render("board-home.html", happinings = happinings)

class HappiningPage(BoardHandler):
	def get(self, happining_id):
		key = db.Key.from_path('Happining', int(happining_id), parent=board_key())
...
class NewHappining(BoardHandler):
	def get(self):
	...
	def post(self):
		...
		if title and description and date and location:
			e = Happining(parent = board_key(), title = title, date = date, location = location, description = description)
			e.put()
			self.redirect('/%s' % str(e.key().id()))
		else:
			error = "You need a title, date, location, and description."
			self.render("new-pin.html", title=title, date=date, location=location, description=description, error = error)
		
class WelcomePage(BoardHandler):
    def get(self):
